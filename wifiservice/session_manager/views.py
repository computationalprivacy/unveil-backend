...
from .utils import (
    stop_session_helper, get_or_create_session,
...
def start_automated_session(request, delay=0):
    ...
    available_pi = get_status_recent_helper(
        settings.DEMO_CONSTANTS.active_pi_threshold)
...
def get_ap(request):
    ...
    if session:
        ...
        ssid_to_send = dict(
            ssid='Experiment-UNVEIL-{}'.format(random.randint(1, 100000)),
            channel=6)
        if sticky == 0:
            ...
            session_collection.replace_one({
                '_id': session['_id']}, session)
        return JsonResponse({
            'ssid': ssid_to_send['ssid'],
            'channel': ssid_to_send['channel']})
    else:
