...
def is_pi_busy(pi):
    ...
    if (pi['is_busy'] and
        (datetime.datetime.now() -
            pi['updated_at']).total_seconds() > settings.MAX_EXECUTION_TIME):
        return False
    if pi['is_busy']:
        return True
    return False
...
def get_pi_instruction_obj(mac_address, get_default=False):
    ...
    pi_instruction_obj = instructions_collection.find_one(
        {'mac': mac_address})
...
def empty_mac_instructions(mac_address):
    ...
    pi_instruction_obj = get_pi_instruction_obj(
        mac_address, get_default=True)
...
def is_instruction_available(mac_address):
    ...
    pi_instruction_obj = get_pi_instruction_obj(mac_address)
    return bool(
        pi_instruction_obj and
        len(pi_instruction_obj['instruction_list']) > 0 and
        not is_pi_busy(pi_instruction_obj))
...
def executed_instruction(request):
    ...
    try:
        ...
        code = instruction['code']
        pi_instruction_obj = get_pi_instruction_obj(mac, get_default=True)
        if (pi_instruction_obj['is_busy'] and
                pi_instruction_obj['instruction_list'][0] == code):
            pi_instruction_obj['is_busy'] = False
