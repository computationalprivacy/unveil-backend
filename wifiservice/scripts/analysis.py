...
def assign_dev_rating(res):
    ...
    for dev in res['results']:
        ...
        if 'rating' not in dev:
            dev['DNS_Queries'] = packets_analyzer.sanitize_dns(
                dev['DNS_Queries'])
...
def analyze_helper(data_path):
    ...
    res = {
        'session_id': session_id, 'path': data_path,
        'creation_time': creation_time,
        'results': analysis_results}
    res = assign_dev_rating(res)
    update = internet_data_collection.replace_one(
        {'path': data_path}, res, upsert=True)