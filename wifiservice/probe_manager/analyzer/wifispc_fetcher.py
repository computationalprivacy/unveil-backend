...
def wifispc_scraping(
        min_long=-4.145883083343500, max_long=1.733506560300000,
        increament=0.1, min_lat=45.833381652832000,
    ...
    for longitude in mit.numeric_range(min_long, max_long, increament):
        ...
        for latitude in bar(mit.numeric_range(min_lat, max_lat, increament)):
            index += 1
            url = 'https://wifispc.com/api/index.php?access_token=adiua;' \
            ...
            if 'coordinates' in json_object:
                for dic in json_object['coordinates']:
                    ...
                    if dic['nwid']:
                        ...
                        if (longitude not in nwid_set and
                                latitude not in nwid_set):
...
def mongo_db_export_json(merged_file, collection_name, db_name):
    ...
    """
    client = MongoClient('localhost', 27017, username='wifi',
                         password='WiFiUnveIL')
    db = client[db_name]
