...
class Wigle(SSIDAddFetcher):
    ...
    def average_location(self, locations, ssid, locations_length):
        ...
        if locations_length == 1:
            ...
            countries_lst = [
                location["country"] for location in locations if location["country"]
            ]
            # have duplicates
            if len(set(countries_lst)) != len(countries_lst):
                # get the most freq country
                # calculate median without other countries
                most_freq_country = max(set(countries_lst), key=countries_lst.count)
                med_lat = median(
                    (
                        location["trilat"]
    ...
    def lookup_ssid(self, ssid):
        ...
        r = requests.get(
            ...
            "freenet=false&paynet=false&ssid=%s" % ssid,
            auth=self.auth,
        )
        if r.status_code != 200:
            logging.error(
                "Unable to lookup %s, bad status: %d. "
                "Have you set your API keys correctly?" % (ssid, r.status_code)
            )
            return
        try:
        ...
        if not locations:
            ...
            return {}
        filtered_location = self.average_location(locations, ssid, len(locations))
