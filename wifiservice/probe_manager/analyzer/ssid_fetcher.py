...
class SSIDAddFetcher(object):
    ...
    def _get_address(self, gps_lat, gps_long):
        ...
        lookup_url = "http://nominatim.openstreetmap.org/reverse?zoom=18&" \
        ...
        try:
            ...
            if (req.status_code == 200 and
                    'json' in req.headers['content-type']):
            ...
            return {
                ...
                'address': longaddress, 'shortaddress': shortaddress,
                'city': city, 'country': country, 'code': country_code,
                'county': county, 'postcode': postcode,
                'road': road, 'state': state, 'suburb': suburb, }
        except Exception as e:
            logging.error(
                "Unable to retrieve address from OpenStreetMap - '{}'".format(
                    str(e)))
